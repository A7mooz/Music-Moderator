module.exports = {
    ...
    modOnly: true,
    callback: ({ arguments, channel, message, prefix, client }) => {

        const timeOut = 1000 * 5

        const user = message.mentions.users.first() || message.guild.member(arguments[1])
        ...
        if (['-r', 'regex'].includes(arguments[0])) {
            if (isNaN(arguments[2])) return message.reply('Please provide a valid number of messages to delete')
            channel.messages.fetch({
                limit: arguments[2]
            }).then((messages) => {
                const rMessages = []

                messages.filter(m => m.content.includes(arguments[1])).forEach(msg => rMessages.push(msg))
